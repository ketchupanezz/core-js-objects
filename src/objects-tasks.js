...
function makeWord(lettersObject) {
  ...
  const maxPos = allPositions.length > 0 ? Math.max(...allPositions) : -1;
  const resultArray = Array(maxPos + 1).fill('');

  Object.entries(lettersObject).forEach(([letter, positions]) => {
    positions.forEach((pos) => {
      resultArray[pos] = letter;
    });
  });
...
function sellTickets(queue) {
  ...
  return queue.every((bill) => {
    ...
    if (bill === 50) {
      if (balance25 === 0) return false;
      balance25 -= 1;
      balance50 += 1;
      return true;
    }
    if (bill === 100) {
      if (balance50 >= 1 && balance25 >= 1) {
        balance50 -= 1;
        balance25 -= 1;
        return true;
      }
